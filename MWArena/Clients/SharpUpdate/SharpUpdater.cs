    ...
    public class SharpUpdater
    {
        ...
        private void DownloadUpdate(SharpUpdateXml update)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                OnUpdatedAppStarted(EventArgs.Empty);
                //Application.Exit();
            }
        ...
        {
            string argument = "/C choice /C Y /N /D Y /T 4 & Del /F /Q \"{0}\" & choice /C Y /N /D Y /T 2 & copy /Y \"{1}\" \"{2}\" & Start \"\" /D \"{3}\" \"{4}\" {5}";
        ...
        /// <summary> Raises the <see cref="E:UpdatedAppStarted" /> event. </summary>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        /// <remarks>Added to support wpf application subscribing to update and shutting down. </remarks>
        protected virtual void OnUpdatedAppStarted(EventArgs e)
        {
            EventHandler handler = UpdatedAppStarted;
            if (handler != null)
            {
                handler(this, e);
            }
        ...
        /// <summary> Occurs when [updated application started]. </summary>
        public event EventHandler UpdatedAppStarted;
